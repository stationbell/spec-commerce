...
const ResolveInput = z
  .strictObject({
    ...
    spec_text: z.string().trim().min(10).max(4000).optional().describe("The specification clause copied EXACTLY as written, numbering and line breaks included (e.g. the FE1 paragraph with its basis of design and alternates); never a rewrite or summary. The page reads it with fixed rules — ratings, capacities, agents, exclusions, basis of design, alternates, combinations — checks this product, searches the catalog per clause, and returns what satisfies it. Anything it could not read comes back as unparsed for you to pass as requirements."),
